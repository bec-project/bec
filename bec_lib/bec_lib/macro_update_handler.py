...
def has_executable_code(content: str) -> tuple[bool, int | None]:
    ...
    Returns:
        bool: True if executable code is found, False otherwise.
        int | None: The line number of the first executable statement found, or None if none found.
    """
    try:
...
class MacroUpdateHandler:
    ...
    def get_macros_from_file(self, file: str) -> dict:
        """
    ...
    def on_macro_update(self, msg: messages.MacroUpdateMessage) -> None:
        ...
        match msg.update_type:
            case "add":
                ...
                self.load_user_macro(msg.file_path, ignore_existing=True)
            case "remove":
                # A single macro is to be removed
                self.forget_user_macro(msg.macro_name)
            case "reload":
                # A single macro is to be reloaded
                self.reload_user_macro(msg.macro_name, msg.file_path)
            case "reload_all":
