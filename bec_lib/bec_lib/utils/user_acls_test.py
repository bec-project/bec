...
class BECAccessDemo:  # pragma: no cover
    ...
    def add_user(self):
        available_user = self.connector._managed_connection._redis_conn.acl_list()
        if f"user {self.username}" in " ".join(available_user):
            self.connector._managed_connection._redis_conn.acl_deluser(self.username)
    ...
    def add_account(self, name: str, password: str | None, level: Literal["admin", "user"]):
        ...
        if f"user {name}" in " ".join(available_user):
            self.connector._managed_connection._redis_conn.acl_deluser(name)
        if level == "admin":
            config = {
                "enabled": True,
                "categories": ["+@all"],
        ...
        self.connector._managed_connection._redis_conn.acl_setuser(name, **config)

    def add_admin(self):
        available_user = self.connector._managed_connection._redis_conn.acl_list()
        if f"user {self.admin_username}" in " ".join(available_user):
            self.connector._managed_connection._redis_conn.acl_deluser(self.admin_username)
        self.connector._managed_connection._redis_conn.acl_setuser(
            self.admin_username,
            enabled=True,
            passwords=["+admin"],
    ...
    def reset(self):
        ...
        self.connector._managed_connection._redis_conn.reset()
        available_user = self.connector._managed_connection._redis_conn.acl_list()
        for user in ["user", "admin", "bec"]:
            if f"user {user}" in " ".join(available_user):
                self.connector._managed_connection._redis_conn.acl_deluser(user)
