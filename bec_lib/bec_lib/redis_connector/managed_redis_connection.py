...
class ManagedRedisConnection:
    ...
    def _get_stream_messages_loop(self) -> None:
        ...
        while not self._stop_stream_events_listener_thread.is_set():
            ...
            stream_errors = {
                error for error in (from_start_error, normal_error) if error is not None
            }
            if stream_errors:  # Encountered an error on xread, wait a while without the lock
                for stream_error in stream_errors - errors:
                    logger.error(stream_error)
                errors = stream_errors
                self._stop_stream_events_listener_thread.wait(timeout=1)
            elif errors and normal_topics:
                # Only a successful Redis read confirms recovery.
                errors.clear()
