...
class StreamSubs:
    def __init__(self) -> None:
        ...
        self._dr_subs: dict[str, dict[DirectReadStreamSubInfo, DirectReadStreamSubInfo]] = {}
    ...
    def all_topics(self):
        with self.lock:
            ...
            dr_sub_keys = [k for k in self._dr_subs if self._dr_subs[k] != set()]
        ...
        for topic in topics_and_end_ids:
            if topic in self._subs:
                if topics_and_end_ids[topic] != self._subs[topic].read_id:
                    ...
                    self._subs[topic].subs.add(self.from_start_subs.pop(topic))  # type: ignore
            else:
    ...
    def add_direct_listener(self, topic: str, new_sub: DirectReadStreamSubInfo):
        ...
        if not topic in self._dr_subs:
            self._dr_subs[topic] = {}
        self._dr_subs[topic][new_sub] = new_sub
    ...
    def remove(self, topic: str, cb: Callable | None = None) -> bool:
        ...
        if cb is None:  # Remove all subs for the given topic
            ...
            if (subs := self._dr_subs.pop(topic, None)) is not None:
                for sub in subs:
        ...
        if topic in self._dr_subs and test_subinfo in self._dr_subs[topic]:
            sub = self._dr_subs[topic].pop(test_subinfo)  # type: ignore # hash is the same
            self._kill_direct_stream(sub, topic)
            removed = True
            if len(self._dr_subs[topic]) == 0:
                del self._dr_subs[topic]
