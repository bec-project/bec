...
class BECClient(BECService):
    ...
    def data_api(self):
        ...
        """
        if self._data_api is None or not getattr(self._data_api, "_initialized", False):
            # Re-create after DataAPI.close(): a closed instance has no plugins
            # and would silently serve dead subscriptions.
