...
@pytest.mark.parametrize(
    "content, invalid_entries",
    [
        pytest.param("123: {}\n", "123", id="non-string-device-name"),
        pytest.param("on: {}\n", "True", id="boolean-on-device-name"),
        pytest.param("yes: {}\n", "True", id="boolean-yes-device-name"),
        pytest.param("motor:\n", "'motor'", id="null-device-config"),
        pytest.param("motor: text\n", "'motor'", id="scalar-device-config"),
        pytest.param("motor: []\n", "'motor'", id="list-device-config"),
        pytest.param(
            "valid: {}\nmotor: text\n123: {}\ndetector:\n",
            "'motor', 123, 'detector'",
            id="multiple-invalid-entries",
        ),
    ],
)
def test_config_helper_rejects_invalid_device_mapping(
    config_helper_for_invalid_file, tmp_path, content, invalid_entries
):
    file_path = tmp_path / "config.yaml"
    file_path.write_text(content, encoding="utf-8")
...
def test_config_helper_limits_fuzzy_suggestions(config_helper, tmp_path):
    candidates = [tmp_path / f"test_config{index}.yaml" for index in range(1, 5)]
    for candidate in candidates:
        candidate.write_text("{}", encoding="utf-8")

    suggestions = config_helper._suggest_config_file_alternatives(
        tmp_path / "test_config.yaml", (".yaml", ".yml")
    )

    assert len(suggestions) == 3
    assert len(set(suggestions)) == 3
    assert set(suggestions) <= set(candidates)
