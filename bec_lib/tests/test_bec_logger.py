...
def test_configure(logger, tmp_path):
    with (
        mock.patch.object(logger, "_update_base_path") as mock_update_base,
        mock.patch.object(logger, "writer_mixin") as mock_writer_mixin,
        mock.patch.object(logger, "_update_sinks") as mock_update_sinks,
    ):
