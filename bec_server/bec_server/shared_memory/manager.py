...
class SharedMemoryManager(BECService):
    ...
    def _allocate_memory(self, request: messages.SharedMemAllocationRequest) -> None:
        ...
        with self.lock:
            ...
            self._publish_allocation_info(self._shared_memory_info)
            logger.info(
                f"Allocated shared memory for client {request.client_id} and signal {request.signal} with descriptor {buff.descriptor}"
            )
    ...
    def _deallocate_memory(self, request: messages.SharedMemDeallocationRequest) -> None:
        ...
        with self.lock:
            ...
            self._publish_allocation_info(self._shared_memory_info)
            logger.info(
                f"Deallocated shared memory for client {request.client_id} and signal {request.signal}"
            )
    ...
    def stop(self) -> None:
        with self.lock:
            ...
            logger.info("Stopped shared memory manager and cleared all shared memory objects.")
