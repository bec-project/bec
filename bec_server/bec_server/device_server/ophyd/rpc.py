"""Ophyd RPC execution, native result serialization and cache updates."""
...
from bec_server.device_server.errors import DisabledDeviceError
from bec_server.device_server.ophyd.serialization import is_serializable
...
class RPCHandler:
    ...
    def __init__(
        self,
        instructions: OphydInstructions,
        *,
        assert_device_is_enabled: Callable[[messages.DeviceInstructionMessage], None],
    ) -> None:
        self.instructions = instructions
        self.device_manager = instructions.device_manager
        self.connector = instructions.connector
        self.requests_handler = instructions.requests_handler
        self._assert_device_is_enabled = assert_device_is_enabled
    ...
    def send_rpc_result_to_client(
        ...
        self.requests_handler.finish_if_untracked(diid, success=True, result=res)
        self.connector.set(
    ...
    def send_rpc_exception(self, exc: Exception, instr: messages.DeviceInstructionMessage):
        ...
        error_info = messages.ErrorInfo(
            ...
            device=self.instructions.get_device_from_exception(exc),
        )
        ...
        diid = instr.metadata.get("device_instr_id", "")
        self.requests_handler.finish_if_untracked(diid, success=False, error_info=error_info)
    ...
    def _handle_set(self, instr: messages.DeviceInstructionMessage) -> Any:
        ...
        if isinstance(obj, ophyd.Signal) and getattr(rpc_var, "__name__", None) in ["set", "put"]:
            ...
            val = self.instructions.convert_value_if_needed(obj, val)
            instr.parameter["args"][0] = val
        res = self._execute_rpc_call(rpc_var, instr)

        if isinstance(res, ophyd.StatusBase):
            res = self.instructions.register_status(res, instr, obj)
        else:
    ...
    def _rpc_read_and_return(self, instr: messages.DeviceInstructionMessage) -> Any:
        ...
        Returns:
            Any: Result of RPC instruction
        """
        res = self.instructions.read_and_update_devices([instr.content["device"]], instr.metadata)
    ...
    def _rpc_read_configuration_and_return(self, instr: messages.DeviceInstructionMessage) -> Any:
        ...
        Returns:
            Any: Result of RPC instruction
        """
        res = self.instructions.read_config_and_update_devices(
    ...
    def _serialize_rpc_response(self, instr: messages.DeviceInstructionMessage, res: Any) -> Any:
        ...
        """
        if isinstance(res, OphydStatus):
            native = res.native_status
            return {
                "type": "status",
                ...
                "done": res.done,
                "settle_time": native.settle_time,
            }
