...
from bec_server.device_server.ophyd.serialization import disable_lazy_wait_for_connection
...
def _run_while_active(callback: Callable) -> Callable:
    """Drain in-flight native callbacks before disabling publication at shutdown."""

    @wraps(callback)
    def guarded(self: DeviceManagerDS, *args, **kwargs):
...
class DSDevice(DeviceBaseWithConfig):
    ...
    def initialize_device_buffer(self, connector):
        ...
        if hasattr(self.obj, "low_limit_travel") and hasattr(self.obj, "high_limit_travel"):
            limits = OphydConfiguration.get_device_limits(self.obj)
        else:
            limits = None
...
class DeviceManagerDS(DeviceManagerBase):
    def __init__(
        ...
        self.configuration = OphydConfiguration(self)
    ...
    def update_config(self, obj: OphydObject, config: dict) -> None:
        """Apply native device configuration through the configuration route.

        Args:
            obj (OphydObject): Device object to update.
            config (dict): Device configuration values to apply.
        """
        self.configuration.apply_device_config(obj, config)
    ...
    def disconnect_device(obj):
        ...
        OphydConfiguration.destroy_device(obj)
    ...
    def publish_device_info(
        ...
        """
        try:
            interface = self.serialization.get_device_info(obj, connect=connect)
            self.connector.set(
            ...
            interface = self.serialization.get_device_info(obj, connect=False)
            self.connector.set(
                MessageEndpoints.device_info(obj.name),
                messages.DeviceInfoMessage(device=obj.name, info=interface),
                pipe,
    ...
    def _device_config_update_callback(self, msg) -> None:
        """Ignore dispatched config updates once shutdown has begun."""
        super()._device_config_update_callback(msg)

    @_run_while_active
    def _update_scan_info(self, msg, **kwargs) -> None:
        """Ignore dispatched scan metadata once shutdown has begun."""
        super()._update_scan_info(msg, **kwargs)

    def shutdown(self) -> None:
        """Release manager resources while leaving the service's transport open."""
        # The inherited destructor can run after a partially failed constructor.
        ...
        with shutdown_lock:
            ...
            if self.config_update_handler:
                try:
                    ...
                    logger.exception("Failed to unregister config requests during shutdown.")
            if self._auto_monitor_update_thread.is_alive():
                self._auto_monitor_update_thread.join()
            for topic, callback in (
                (MessageEndpoints.device_config_update(), self._device_config_update_callback),
                (MessageEndpoints.scan_status(), self._update_scan_info),
            ):
                try:
                    self.connector.unregister(topic, cb=callback)
                except Exception:  # pylint: disable=broad-except
                    logger.exception("Failed to unregister manager callback during shutdown.")
            for device in self.devices.values():
                try:
                    ...
                    logger.error(
                        f"Failed to disconnect device {device.name}: {traceback.format_exc()}"
                    )
            self.devices.flush()
