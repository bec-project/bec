...
class PathOptimizerMixin:
    ...
    def optimize_shell(
        ...
        if dr is None:
            # Improved estimation: use median nearest neighbor distance scaled by point density
            tree = cKDTree(pos[:, :2])
            dists, _ = tree.query(pos[:, :2], k=min(5, len(pos)))  # Check up to 5 nearest neighbors
            median_nn = np.median(dists[:, 1:])  # Skip self-distance
            dr_default = max(median_nn * 2.0, max_rad / min(50, max(10, int(np.sqrt(len(pos))))))
        else:
            dr_default = dr
