...
class LineSweepScan(ScanBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        start to stop. The readout is triggered by updates on the readback signal of the moving device
        and may be throttled by setting a minimum update time. If a maximum update time is set, the readout
        will be triggered if the time since the last update exceeds the maximum update time, even if no new update has been received.
    ...
    def scan_core(self):
        ...
        try:
            ...
            while not status.done:
                update_received = self._readback_update_event.wait(timeout=0.05)

                # If no update has been received, check if the
                # maximum update time has been exceeded. If not,
                # continue waiting for updates.
                if not update_received:
                    if self.max_update <= 0 or (time.time() - last_update_time) <= self.max_update:
                        continue
                    # We've exceeded the maximum update time, so pretend we've received an update
                ...
                # Trigger the main readout logic.
                self.at_each_point()
