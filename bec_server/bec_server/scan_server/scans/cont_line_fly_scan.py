"""
Continuous fly-scan implementation for sweeping one motor across a line trajectory.
...
class ContLineFlyScan(ScanBase):
    ...
    scan_name = "_v4_cont_line_fly_scan"
    required_kwargs = ["relative"]
    gui_config = {
        "Device": ["motor", "start", "stop"],
        "Scan Parameters": ["exp_time", "frames_per_trigger", "relative"],
    }
    ...
    def __init__(
        ...
        motor: DeviceBase,
        start: Annotated[
            float, ScanArgument(display_name="Start Position", reference_units="motor")
        ],
        stop: Annotated[float, ScanArgument(display_name="Stop Position", reference_units="motor")],
        exp_time: Annotated[
        ...
        **kwargs,
        ...
        """
        A continuous line fly scan. Use this scan if you want to move a motor
        continuously from start to stop position while acquiring data as fast as
        possible, respecting the exposure time.

        Args:
            motor (DeviceBase): motor to move continuously
        ...
        Examples:
            >>> scans.cont_line_fly_scan(dev.motor1, -5, 5, exp_time=0.05, relative=True)
        """
        super().__init__(**kwargs)
        motor = self.dev[motor] if isinstance(motor, str) else motor
        self.motor = motor
        self.motors = [motor]
        self.start = start
        self.stop = stop
    ...
    def prepare_scan(self):
        ...
        # We start with the baseline readout
        self._baseline_readout_status = self.actions.read_baseline_devices(wait=False)

        # Move the motor to the start position
        self._premove_motor_status = self.actions.set(self.motor, self.positions[0][0], wait=False)

        # Inform potential clients that they should listen to the readback for this motor
        self.actions.add_scan_report_instruction_readback(
            devices=self.motors, start=[self.positions[0][0]], stop=[self.positions[1][0]]
        )
    ...
    def scan_core(self):
        ...
        status = self.actions.set(self.motor, self.positions[1][0], wait=False)
    ...
    def at_each_point(self):
        ...
        This hook allows concrete fly-scan variants to extend or override the
