...
def nd_grid_positions(axes: list[tuple[float, float, int]], snaked: bool = True) -> np.ndarray:
    ...
    Please note that the first axis in the list is the fastest changing axis, and the last axis is the slowest changing axis.
    ...
    positions = np.asarray(_get_positions_recursively(_axes_arrays[::-1]), dtype=float)
    return positions[:, ::-1]
