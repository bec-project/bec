"""
List scan implementation for explicit point-by-point motor trajectories.
...
class ListScan(ScanBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Examples:
            >>> scans.list_scan(dev.motor1, [0, 1, 2], dev.motor2, [4, 3, 2], exp_time=0.1, relative=True)
        ...
        lengths = {len(positions[0]) for positions in self.motor_input_bundles.values()}
        if len(lengths) > 1:
    ...
    def prepare_scan(self):
        ...
        self.positions = np.vstack(
            tuple(positions[0] for positions in self.motor_input_bundles.values())
        ).T.astype(float)
