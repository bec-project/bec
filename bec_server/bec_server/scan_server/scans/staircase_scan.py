"""
Line scan implementation for one or more motors with evenly spaced step positions.
...
class NdStaircaseScan(ScanBase):
    ...
    scan_name = "_v4_nd_staircase_scan"
    ...
    def __init__(
        ...
        *args,
        start: Annotated[
            float, ScanArgument(display_name="Start Position", units=Units.mm)
        ],
        stop: Annotated[
            float, ScanArgument(display_name="Stop Position", units=Units.mm)
        ],
        ...
        settling_time_after_trigger: Annotated[
            float,
            ScanArgument(
                display_name="Settling Time After Trigger", units=Units.s, ge=0
            ),
        ] = 0,
        ...
        **kwargs,
        ...
        self.motor_input_bundles = bundle_args(
            args, bundle_size=self.arg_bundle_size["bundle"]
        )
    ...
    def prepare_scan(self):
        ...
        def staircase_nd(start, end, n, k):
            s = (end - start) / n
            v = start + s * np.arange(n)
            i = np.arange(n)
            return np.column_stack([v - s * ((i + (k - j)) % k != 0) for j in range(k)])

        self.positions = staircase_nd(
            self.start, self.stop, self.steps, len(self.motors)
        )
        ...
        self._premove_motor_status = self.actions.set(
            self.motors, self.positions[0], wait=False
        )
    ...
    def at_each_point(
        ...
        """
        self.components.step_scan_at_each_point(
            motors, positions, last_positions=last_positions
        )
