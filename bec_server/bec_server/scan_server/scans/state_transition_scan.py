...
class StateTransitionScan(ScanBase):
    ...
    def scan_core(self):
        ...
        logger.warning(f"Request moving motors {motors}.")
    ...
    def _fetch_devices_signals_and_limits_to_set(
        ...
        list[Tuple[Signal, Any]],
        dict[str, Tuple[Positioner, float, float]],
        list[Tuple[Positioner, float]],
    ]:
        """
        ...
        for req in requirements:
            ...
            try:
                expected_value = AggregatedState.get_expected_value(req, self.device_manager)
            except Exception as exc:
                raise StateTransitionScanError(
                    exc_type="ExpectedValueError",
                    message=f"Failed to get expected value for device {req.device_name} and signal {req.signal_name} with expected value {req.expected_value} or user_parameter {req.at}.",
                    compact_message=f"Failed to get expected value for device {req.device_name}.",
                ) from exc
            # First we handle Signals logic
            if isinstance(dev_obj, Signal):
                _signals_to_set.append((dev_obj, expected_value))
                continue
            # Device logic. Devices must implement .set for this to work, otherwise we can not set them and we raise an error
            if isinstance(dev_obj, DeviceBase):
                # First we handle logic for signals (signal_name == device_name) aswell as motors, e.g. samx, samx
                if req.signal_name == req.device_name:
                    _devices_to_set.append((dev_obj, expected_value))
                    continue
                if req.source == "limits":
                    # Check that class supports limits interface with low_limit and high_limit attributes. If not, raise an error.
                    if (
                        not isinstance(dev_obj, (Positioner, Signal))
                        or not hasattr(dev_obj, "low_limit")
                        or not hasattr(dev_obj, "high_limit")
                    ):
                        raise StateTransitionScanError(
                            exc_type="DeviceNotPositionerOrSignal",
                            message=f"Device {req.device_name} is not a Positioner or Signal, but is required to set limits.",
                            compact_message=f"Device {req.device_name} is not a Positioner or Signal.",
                        )
