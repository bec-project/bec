...
class GeneratorScanWorker:
    """
    Scan worker class that processes scan instructions and sends device instructions to the device server.
    """
    ...
    def open_scan(self, instr: messages.DeviceInstructionMessage) -> None:
        ...
        if not self.scan_id:
            ...
            if instr.content["parameter"].get("scan_motors") is not None:
                self.scan_motors = [
                    self.worker.device_manager.devices[dev]
        ...
        active_rb = self.worker.current_instruction_queue_item.active_request_block
    ...
    def close_scan(self, instr: messages.DeviceInstructionMessage, max_point_id: int) -> None:
        ...
        if scan_info.get("scan_type") == "fly":
            ...
            num_points = (
                self.worker.current_instruction_queue_item.active_request_block.scan.num_pos
            )
    ...
    def publish_data_as_read(self, instr: messages.DeviceInstructionMessage):
        ...
        Args:
            instr (DeviceInstructionMessage): Device instruction received from the scan assembler
        """
        connector = self.worker.device_manager.connector
    ...
    def forward_instruction(self, instr: messages.DeviceInstructionMessage) -> None:
        ...
        """
        self.worker.connector.send(MessageEndpoints.device_instructions(), instr)
    ...
    def scan_report_instructions(self):
        ...
        """
        req_block = self.worker.current_instruction_queue_item.active_request_block
        return req_block.scan_report_instructions

    def _wait_for_device_server(self) -> None:
        self.worker.parent.wait_for_service("DeviceServer")
    ...
    def _initialize_scan_info(
        ...
        self.current_scan_info.update(
            {
                "scan_number": self.worker.parent.scan_number,
                "dataset_number": self.worker.parent.dataset_number,
                ...
                "file_components": compile_file_components(
                    ...
                    scan_nr=self.worker.parent.scan_number,
        ...
        self.current_scan_info["scan_msgs"] = [
            str(scan_msg) for scan_msg in self.worker.current_instruction_queue_item.scan_msgs
        ]
        ...
        self.current_scan_info["readout_priority"] = {
            "monitored": [
                dev.full_name
                for dev in self.worker.device_manager.devices.monitored_devices(
                    readout_priority=self.readout_priority
                )
            ],
            "baseline": [
                dev.full_name
                for dev in self.worker.device_manager.devices.baseline_devices(
                    readout_priority=self.readout_priority
                )
            ],
            "async": [
                dev.full_name
                for dev in self.worker.device_manager.devices.async_devices(
                    readout_priority=self.readout_priority
                )
            ],
            "continuous": [
                dev.full_name
                for dev in self.worker.device_manager.devices.continuous_devices(
                    readout_priority=self.readout_priority
                )
            ],
            "on_request": [
                dev.full_name
                for dev in self.worker.device_manager.devices.on_request_devices(
                    readout_priority=self.readout_priority
                )
            ],
        }
    ...
    def _get_file_base_path(self) -> str:
        ...
        """
        current_account_msg = self.worker.connector.get_last(MessageEndpoints.account(), "data")
    ...
    def process_instructions(self, queue: InstructionQueueItem) -> None:
        """
        ...
        try:
            ...
            with self.worker.device_manager._rpc_method(rpc_method):
                for instr in queue:
    ...
    def cleanup(self):
        ...
        status = ScanStubStatus(self.worker.parent.queue_manager.instruction_handler)
        staged_devices = [
            dev.root.name for dev in self.worker.device_manager.devices.enabled_devices
        ]
