...
class BeamlineStateManager:
    ...
    def __init__(self, connector: RedisConnector, device_manager: DeviceManagerBase) -> None:
    ...
    def update_states(self, msg: messages.AvailableBeamlineStatesMessage) -> None:
        ...
        for state_name in remove_state_names:
            ...
            state = self._states.pop(state_name, None)
            if state is not None and state.started:
                try:
                    ...
                    info = ErrorInfo(
                        exception_type=type(exc).__name__,
                        error_message=content,
                        compact_error_message=f"Error stopping beamline state {state_name}.",
                    )
