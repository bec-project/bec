...
class ScanActions:
    ...
    def stage_all_devices(self, wait=True) -> ScanStubStatus:
        ...
        """
        status = self._create_status(name="stage_all_devices")
    ...
    def trigger_all_devices(self, min_wait: float | None = None, wait=True) -> ScanStubStatus:
        ...
        instr = messages.DeviceInstructionMessage(
            device=None,
            action="trigger",
            parameter={"min_wait": min_wait} if min_wait is not None else {},
            metadata={"device_instr_id": status._device_instr_id},
        )
        self._send(instr)
    ...
    def add_scan_report_instruction_readback(
        ...
        self._scan._scan_report_instructions.append(scan_report_instruction)
    ...
    def __init__(
        ...
        for kwarg in ["metadata", "user_metadata", "scan_queue"]:
            if kwarg is not None:
                optional_kwargs[kwarg] = locals()[kwarg]
        ...
        self._scan_report_instructions = []
