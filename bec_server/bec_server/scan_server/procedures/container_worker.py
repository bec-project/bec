    ...
    def _setup_execution_environment(self):
        try:
            ...
            if e.status_code == HTTPStatus.INTERNAL_SERVER_ERROR:
                raise NoImage(
                    f"Podman could not resolve image name: {IMAGE_NAME} - try rebuilding or pulling the BEC container image."
                ) from e
            raise NoPodman(
                f"Could not connect to podman socket at {PODMAN_URI} - is the systemd service running? Try `systemctl --user start podman.socket`."
            ) from e
