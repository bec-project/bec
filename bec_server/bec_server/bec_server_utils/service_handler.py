...
DefaultTmuxSession = TmuxSession()
...
class ServiceDesc:
    path: Template
    command: str
    tmux_session: TmuxSession = DefaultTmuxSession
    ...
    def start(self) -> list:
        ...
        if self.start_redis:
            if self.config_path:
                ...
                redis_ext_modules_path = (
                    pathlib.Path(os.environ.get("CONDA_PREFIX", "/usr")) / "lib"
                )
                ...
                services.update(
                    {
                        ...
                        "redis-data-memory-tracker": ServiceDesc(
                            Template(""),
                ...
                raise RuntimeError("Cannot start Redis without a configuration file")
