...
def test_scan_hook_impl_rejects_non_string_scan_name_patterns():
    with pytest.raises(ValueError, match="scan_names must contain only string"):
        scan_hook_impl("stage", "before", ["line_scan", 1])  # type: ignore[list-item]
