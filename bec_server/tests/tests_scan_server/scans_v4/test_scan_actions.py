...
def test_broadcast_bec_signal_info_emits_owned_devices(action_context):
    ...
    status = mock.MagicMock(_device_instr_id="diid-123")
    ...
    assert msg.metadata["device_instr_id"] == "diid-123"
    assert msg.metadata["RID"] == "rid-123"
    assert msg.metadata["scan_id"] == "scan-id-test"
    status.wait.assert_called_once_with()


def test_broadcast_bec_signal_info_emits_without_owned_devices(action_context):
    ctx = action_context()
    ctx.scan.scan_info.metadata["RID"] = "rid-123"
    ctx.device_manager.parent.device_lock_registry = mock.MagicMock()
    ctx.device_manager.parent.device_lock_registry.get_owned_devices.return_value = []
    status = mock.MagicMock(_device_instr_id="diid-empty")
    ctx.actions._create_status = mock.MagicMock(return_value=status)
    ...
    assert msg.device == []
    assert msg.parameter == {"scan_id": "scan-id-test"}
    assert msg.metadata["device_instr_id"] == "diid-empty"
    status.wait.assert_called_once_with()
