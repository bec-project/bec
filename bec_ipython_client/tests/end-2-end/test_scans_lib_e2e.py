...
def test_config_updates(bec_client_lib):
    ...
    assert dev.rt_controller.velocity.read(cached=False)["rt_controller-velocity"]["value"] == 10
    assert dev.rt_controller.read_configuration()["rt_controller-velocity"]["value"] == 10
    assert (
        dev.rt_controller.read_configuration(cached=False)["rt_controller-velocity"]["value"] == 10
    )
