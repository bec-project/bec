...
In general, there are two fundamentally different types of devices in Ophyd, `Signal` and `Device`. They represent the fundamental building blocks of Ophyd and will be further explained in the following. 
...
### Device
Devices are more complex building blocks of Ophyd. They represent a hierarchy of signals and devices, meaning that can be composed of multiple signals and potentially sub-devices. For example, the `EpicsMotor` device includes signals such as `user_setpoint`, `user_readback`, `motor_is_moving`, and `motor_done_move`. These signals are all part of the motor device and can be accessed through the motor object. More complex devices, such as detectors, may be composed of various components used to configure and prepare the detector for an upcoming acquisition. Signals and sub-devices are implemented as [*components*](https://nsls-ii.github.io/ophyd/generated/ophyd.device.Component.html) of the device, which can be accessed through the device object. Further details can be found in the [Ophyd documentation](https://nsls-ii.github.io/ophyd/device-overview.html). 

As mentioned above, devices implement an abstract hardware interface consisting of a set of methods and properties. In the following, we would like to briefly introduce some of these methods in the context of devices and signals.
